...
$page_header = cnv_option( 'page_header', true );
$page_header_img = cnv_option( 'page_header_image' );
$page_header_title = cnv_option( 'page_header_default_title' );
$page_header_crumb = cnv_option( 'page_breadcrumbs', true );
$support_signle_title = cnv_option( 'support_single_title', true );
...
if ( is_singular() ) {
	...
	} elseif ( is_singular( 'cnv_support' ) ) {
		if ( !empty ( $support_signle_title ) ) {
			$banner_title = $support_signle_title;
		} else {
			$banner_title = esc_html__( 'Support Details', 'cnvschool' );
		}
	} else {

		global $post;

		$meta = get_post_meta( $post->ID, 'cnv_page_options', true );
	...
	if ( have_posts() ) {
		$banner_title = sprintf( esc_html__( 'Search Results for: %s', 'cnvschool' ), '<span>' . get_search_query() . '</span>' );
	} else {
		$banner_title = sprintf( esc_html__( 'Search Results for: %s', 'cnvschool' ), '<span>' . get_search_query() . '</span>' );
	}
	...
	if ( !empty( $postId ) ) {
		$meta = get_post_meta( $postId, 'cnv_page_options', true );
		if ( !empty( $meta['custom_title'] ) ) {
			$banner_title = $meta['custom_title'];
		} else {
	...
	$banner_title = esc_html__( '404', 'cnvschool' );
} else {
	$banner_title = esc_html__( 'Blog', 'cnvschool' );
}
			...
			<?php if ( $banner_crumb == true ) { ?>
                <div class="breadcrumb-wrapper">
                    <div class="breadcrumb-inner">
						<?php echo CNV_Theme_Helper::cnv_breadcrumb(); ?>
