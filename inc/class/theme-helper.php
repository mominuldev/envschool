...
/**
 * CNV Theme Helper
 * @class        CNV_Theme_Helper
 * @version      1.0
 * @category     Class
 * @author       CodeNest Ventures
 */

if ( ! class_exists( 'CNV_Theme_Helper' ) ) {
	class CNV_Theme_Helper {
		...
		function __construct() {
			add_action( 'cnv_after_body', array( $this, 'cnv_preloader_markup' ), 1 );
			add_action( 'cnv_after_body', [ $this, 'cnv_backtotop' ] );
		}

		public function cnv_backtotop() {
			$backtotop = cnv_option( 'back_to_top_switch' );
			if ( $backtotop == '1' ) { ?>
                    ...
                    <div class="progress-wrap active-progress">
                        <svg class="progress-circle svg-content" width="100%" height="100%" viewBox="-1 -1 102 102">
                            <path d="M50,1 a49,49 0 0,1 0,98 a49,49 0 0,1 0,-98"
                                  style="transition: stroke-dashoffset 10ms linear 0s; stroke-dasharray: 307.919, 307.919; stroke-dashoffset: 23.9589;"></path>
                        </svg>
                    </div>
		...
		public static function options_compare( $name, $check_key = false, $check_value = false ) {
			...
			if ( $check_key ) {
				...
				$var    = $meta[ $name ];
				$option = ! empty( $var ) ? $meta[ $name ] : cnv_option( $name );
			}
		 ...
		 */
		//Add breadcrumbs
		static function cnv_breadcrumb() {
			global $post;
			$cnv_opt  = get_option( 'cnv_opt' );
			...
			if ( ! is_home() ) {
				...
				if ( is_category() || is_single() ) {
					...
					printf( esc_html__( 'Archive for: %s', 'cnvschool' ), '<span>' . get_the_date() . '</span>' );
				} elseif ( is_month() ) {
					printf( esc_html__( 'Archive for: %s', 'cnvschool' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'cnvschool' ) ) . '</span>' );
				} elseif ( is_year() ) {
					printf( esc_html__( 'Archive for: %s', 'cnvschool' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'cnvschool' ) ) . '</span>' );
				} elseif ( is_author() ) {
					echo "<span>" . esc_html__( 'Archive for', 'cnvschool' );
					echo '</span>';
				} elseif ( isset( $_GET['paged'] ) && ! empty( $_GET['paged'] ) ) {
					echo "<span>" . esc_html__( 'Blog Archives', 'cnvschool' );
					echo '</span>';
				} elseif ( is_search() ) {
		 ...
		 * @since cnvschool 1.0
		 */
		static function cnv_content_nav( $html_id ) {
			global $wp_query;
			$html_id = esc_attr( $html_id );
			if ( $wp_query->max_num_pages > 1 ) : ?>
                <nav id="<?php echo esc_attr( $html_id ); ?>" class="navigation" role="navigation">
                    <h3 class="assistive-text"><?php esc_html_e( 'Post navigation', 'cnvschool' ); ?></h3>
                    <div
                            class="nav-previous"><?php next_posts_link( wp_kses( __( '<span class="meta-nav">&larr;</span> Older posts', 'cnvschool' ), array( 'span' => array( 'class' => array() ) ) ) ); ?></div>
                    <div
                            class="nav-next"><?php previous_posts_link( wp_kses( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'cnvschool' ), array( 'span' => array( 'class' => array() ) ) ) ); ?></div>
                </nav>
			<?php endif;
		}
		...
		static function cnv_post_pagination( $nav_query = false ) {
		 ...
		 * simply create your own cnv_comment(), and that function will be used instead.
		 * Used as a callback by wp_list_comments() for displaying the comments.
		 * @since cnvschool 1.0
		 */
		static function cnv_comment( $comment, $args, $depth ) {
			...
			switch ( $comment->comment_type ) :
				...
				case 'trackback' :
                    ...
                    <p><?php esc_html_e( 'Pingback:', 'cnvschool' ); ?><?php comment_author_link(); ?><?php edit_comment_link( esc_html__( '(Edit)', 'cnvschool' ), '<span class="edit-link">', '</span>' ); ?></p>
					<?php
					break;
				default :
                        ...
                        <div class="comment-info">
                            <header class="comment-meta comment-author vcard">
								...
								printf( '<cite><b class="fn">%1$s</b> %2$s</cite>', get_comment_author_link(), // If current post author is also comment author, make it known visually.
									( $comment->user_id === $post->post_author ) ? '<span>' . esc_html__( 'Post author', 'cnvschool' ) . '</span>' : '' );
								printf( '<time datetime="%1$s">%2$s</time>', get_comment_time( 'c' ), /* translators: 1: date, 2: time */ sprintf( esc_html__( '%1$s at %2$s', 'cnvschool' ), get_comment_date(), get_comment_time() ) );
								?>
                                <div class="reply">
									<?php comment_reply_link( array_merge( $args, array(
										'reply_text' => esc_html__( 'Reply', 'cnvschool' ),
										'after'      => '',
										'depth'      => $depth,
							...
							<?php if ( '0' == $comment->comment_approved ) : ?>
                                <p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'cnvschool' ); ?></p>
							<?php endif; ?>
                            <section class="comment-content comment">
								<?php comment_text(); ?>
								<?php edit_comment_link( esc_html__( 'Edit', 'cnvschool' ), '<p class="edit-link">', '</p>' ); ?>
		 ...
		 * Create your own cnv_entry_meta() to override in a child theme.
		 * @since cnvschool 1.0
		 */
		static function cnv_entry_meta() {
			...
			if ( comments_open() ) {
				if ( $num_comments == 0 ) {
					$comments = esc_html__( '0 comments', 'cnvschool' );
				} elseif ( $num_comments > 1 ) {
					$comments = $num_comments . esc_html__( ' comments', 'cnvschool' );
				} else {
					$comments = esc_html__( '1 comment', 'cnvschool' );
				}
				$write_comments = '<a href="' . get_comments_link() . '">' . $comments . '</a>';
			}
			$utility_text = NULL;
			if ( ( post_password_required() || ! comments_open() ) && ( $tag_list != false && isset( $tag_list ) ) ) {
				$utility_text = esc_html__( 'Tags: %2$s', 'cnvschool' );
			} elseif ( $tag_list != false && isset( $tag_list ) && $num_comments != 0 ) {
				$utility_text = esc_html__( '%1$s / Tags: %2$s', 'cnvschool' );
			} elseif ( ( $num_comments == 0 || ! isset( $num_comments ) ) && $tag_list == true ) {
				$utility_text = esc_html__( 'Tags: %2$s', 'cnvschool' );
			} else {
				$utility_text = esc_html__( '%1$s', 'cnvschool' );
			}
			printf( $utility_text, $write_comments, $tag_list );
		}

		static function cnv_entry_meta_small() {
			// Translators: used between list items, there is a space after the comma.
			$categories_list = get_the_category_list( ', ' );
			$author          = sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>', esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ), esc_attr( sprintf( wp_kses( __( 'View all posts by %s', 'cnvschool' ), array( 'a' => array() ) ), get_the_author() ) ), get_the_author() );
			$utility_text    = esc_html__( 'By %1$s', 'cnvschool' );
        ...
        // Posted on date format 2, Date, Full Month, Year
        static function cnv_posted_date() {
            $post_date = get_the_date();
            $date_parts = explode(' ', $post_date);

            // Remove the comma from the date
            $date_parts[1] = str_replace(',', '', $date_parts[1]);

            if (count($date_parts) === 3) {
                $day = '<span class="post-day">' . esc_html($date_parts[1]) . '</span>';
                $monthDate = '<span class="post-month-date">' . esc_html($date_parts[0]). ' ' . esc_html($date_parts[2]) . '</span>';
		...
		static function cnv_entry_cat() {
			// Hide category and tag text for pages.
			if ( 'post' === get_post_type() ) {
				/* translators: used between list items, there is a space after the comma */
				$categories_list = get_the_category_list( esc_html__( ' ', 'cnvschool' ) );
				if ( $categories_list ) {
					/* translators: 1: list of categories. */
					printf( esc_html__( ' %1$s', 'cnvschool' ), $categories_list ); // WPCS: XSS OK.
				}
		...
		static function cnv_posted_tag() {
			// Hide category and tag text for pages.
			if ( 'post' === get_post_type() ) {

				/* translators: used between list items, there is a space after the comma */
				$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'cnvschool' ) );
				if ( $tags_list ) {
					/* translators: 1: list of tags. */
					printf( '<div class="single-tagcloud"><span class="tag-title">' . esc_html( 'Tags:', 'cnvschool' ) . '</span>' . esc_html__( '%1$s', 'cnvschool' ) . '</div>', $tags_list ); // WPCS: XSS OK.
				}
			}
		}
		 ...
		 * Trim text
		 */
		static function cnv_substring( $string, $limit, $afterlimit = '[...]' ) {
			if ( empty( $string ) ) {
				return $string;
			}
		 ...
		 * @since cnvschool 1.0
		 */
		public function cnv_get_footer_column( $footer_columns ) {
		...
		public static function render_sidebars( $args = 'blog' ) {
			...
			$layout        = cnv_option( $args . '_sidebar_layout' ) ? cnv_option( $args . '_sidebar_layout' ) : 'right';
			$sidebar       = 'sidebar_main-sidebar';
			$sidebar_width = cnv_option( $args . '_sidebar_def_width', '8' );
			$sidebar_gap   = cnv_option( $args . '_sidebar_gap', '45' );
		...
		private function post_author( $author ) {
			...
			return '<span class="post_author">' . esc_html__( "by", "cnvschool" ) . ' <a href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author_meta( 'display_name' ) ) . '</a></span>';
		}
		...
		public static function post_view_count( $id ) {
			...
			return '<p class="post_view_count">' . esc_html( $count ) . '<span>' . esc_html__( " Views", "cnvschool" ) . '</span></p>';
		 ...
		 */
		static function cnv_post_thumbnail() {
		...
		static function render_author_info() {
			...
			if ( (bool) $name_html || (bool) $description || (bool) $social_html ) {
                echo '<h3 class="author-info_title">'.esc_html__('About Author','cnvschool').'</h3>';
		...
		public function cnv_preloader_markup() {
			$preloader_switch = cnv_option( 'preloader_switch' );
			$preloader_opt    = cnv_option( 'preloader' );
			$preloader_type   = cnv_option( 'preloader-type' );
			$preloader_img    = cnv_option( 'preloader-images' );
			$preloader_text   = cnv_option( 'preloader_text' );
		...
		static function cnv_entry_comments( $post_id, $no_comments = 'No Comment' ) {

			$comments_number = get_comments_number( $post_id );
			if ( $comments_number == 0 ) {
				$comment_text = $no_comments;
			} elseif ( $comments_number == 1 ) {
				$comment_text = esc_html__( '1 Comment', 'cnvschool' );
			} elseif ( $comments_number > 1 ) {
				$comment_text = $comments_number . '<span class="comment-text">' . esc_html__( ' Comments', 'cnvschool' ) . '</span>';
			}
		...
		public static function branding_logo() {
			...
			$logo_main        = cnv_option( 'main_logo' );
			$sticky           = cnv_option( 'sticky_logo' );
			$retina_logo      = cnv_option( 'retina_logo' );
			$retina_sticky    = cnv_option( 'retina_logo_sticky' );
			$is_sticky_header = cnv_option( 'header_sticky' );
			$transparent_logo = cnv_option( 'transparent_main_logo' );
		...
		static function related_post() { ?>
            <div class="related-post-wrapper">
                <h2 class="related-title"><?php echo esc_html( cnv_option( 'related_title' ) ); ?></h2>
                <div class="row">
					<?php
					global $post;
					$post_id    = get_the_ID();
                                ...
                                <div class="related-post">
                                    ...
                                    <div class="blog-content">
                                        ...
                                        <ul class="post-footer-meta">
                                            ...
                                            <li><i class="feather-calendar"></i><?php CNV_Theme_Helper::cnv_posted_on(); ?></li>
		...
		static function cnv_post_nav() {
			...
			if ( ! empty( $pre_post ) ):
                ...
                <a href="<?php echo get_the_permalink( $pre_post->ID ); ?>" class="single-post-nav">
                    ...
                    <div class="post-nav-wrapper">
                        <p class="post-nav-title"><?php esc_html_e( 'Older Post', 'cnvschool' ) ?></p>
			...
			if ( ! empty( $next_post ) ):
                ...
                <a href="<?php echo get_the_permalink( $next_post->ID ); ?>" class="single-post-nav">
                    ...
                    <div class="post-nav-wrapper">
                        <p class="post-nav-title"><?php esc_html_e( 'Next Post', 'cnvschool' ) ?></p>
		...
		static function cnv_portfolio_post_nav() {
			// Don't print empty markup if there's nowhere to navigate.
			$pre_post  = $next_post = '';
			$next_post = get_next_post();
			$pre_post  = get_previous_post();
		 ...
		 * @return array
		 */
		static function cnv_get_title_tag( $first_empty = false, $additional_elements = array() ) {
			$title_tag = array();
		...
		public static function get_footers_types() {
			$footer  = [ '' => esc_html__( 'Default', 'cnvschool' ) ];
			$footers = get_posts(
				[
					'posts_per_page' => - 1,
					'post_type'      => 'cnv_footer',
					'orderby'        => 'name',
					'order'          => 'ASC'
				]
			);
		...
		public static function cnv_render_footer( $footer_style ) {
			$elementor_instance = Elementor\Plugin::instance();

			return $elementor_instance->frontend->get_builder_content_for_display( $footer_style );
		}

		static public function cnv_excerpt( $limit ) {
